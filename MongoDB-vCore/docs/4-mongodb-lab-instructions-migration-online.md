...
3. A 7-step migration wizard will appear on the right-hand side of your screen to guide you through the whole migration without needing to run any commands.
   ...
   3. Cutover: Once the source and target servers are nearly in full sync*, we repoint our application to the target. Technically speaking, in presence of continued writes/updates/deletes on the source server, the target server will always lag behind by a small delta. The idea here is to minimize this delta to seconds, or max few minutes range. Please note that the lowest achievable delta also depends on the given migration tool. Most migration tools synchronize changes from source to target in microbatches that may execute only every few seconds or perhaps every minute to reduce strain on source server. Once delta is minimized, and we are ready to proceed with swap, we have two options or I shall say tradeoffs:
       1. we may prioritize full safety and data consistency - in such case, we would stop our application for a very brief moment (~1 minute), wait for the latest changes to synchronize to target (should only take some seconds as per the frequency of microbatching), verify document counts match on source and target, and repoint our application
       2. we may prioritize uptime - in such case, we perform a rolling upgrade (e.g., [k8s](https://kubernetes.io/docs/tutorials/kubernetes-basics/update/update-intro/)) and bring up new instance of our application with connection string pointing to target server while gradually draining connections from source. This approach can achieve zero downtime, but it makes it harder to reason about consistency of data during the cutover as well as execute rollback, if needed.

   >[!note] Unlike in previous migration attempt, this time we are leaving our application running. Our users can continue placing orders on our website all the while we are upgrading our database backend. When the time to perform a cutover comes, we will, however, choose to execute the simpler cutover and stop our app for very brief moment to verify data is fully in sync on both servers.
